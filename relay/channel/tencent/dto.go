...
type TencentChatRequest struct {
	...
	// 3. Messages 和 Content 总长度不能超过模型输入长度上限（可参考[产品概述](https://cloud.tencent.com/document/product/1729/104753) 文档），超过则会截断最前面的内容，只保留尾部内容
	...
	//
	// 注意：
	// 通过 SDK 调用时，流式和非流式调用需要*不同的方法*获取返回值，具体参考 SDK 中的注释或示例（在各语言 SDK 代码仓库 examples/hunyuan/v20230901/ 目录中）
...
type TencentResponseChoices struct {
	FinishReason string         `json:"FinishReason,omitempty"` // 流式结束标志位，为 stop 则表示尾部
...
type TencentChatResponse struct {
	...
	ReqID   string                   `json:"Req_id,omitempty"`  // 唯一请求 Id，每次请求都会返回。用于反馈接口输入
}
